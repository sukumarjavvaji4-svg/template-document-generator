...
async function analyzeDocx(filePath, label) {
  ...
  console.log(`\n==================================================`);
  console.log(`PARAGRAPH ANALYSIS FOR ${label}`);
  console.log(`==================================================`);
  ...
  while ((pm = pRe.exec(docXml)) !== null) {
    ...
    if (numPrMatch) {
      ...
      if (text.includes('Every learner shall register') || text.includes('Learners shall be able to export') || text.includes('Only registered learners')) {
